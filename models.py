...
#            # Observation size + one-hot of previous action + previous reward.
#            nn.Linear(observation_size + self.action_size + 1, hidden_size),
#            nn.ReLU(True),
#        )
...
#
#    def forward(self, observation, previous_action, previous_reward, rnn_state=None):
#        time_size, batch_size, _ = observation.shape
#
#        time_batch_merged_observation = torch.flatten(observation, 0, 1)
#
#        one_hot_last_action = F.one_hot(previous_action.view(time_size * batch_size), self.action_size).float()
#        clipped_reward = torch.clamp(previous_reward, -1, 1).view(time_size * batch_size, 1)
#        core_input = torch.cat([time_batch_merged_observation, one_hot_last_action, clipped_reward], dim=-1)
#
